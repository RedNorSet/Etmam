...
_log_sub   = logging.getLogger('gpms.submissions')
_log_grade = logging.getLogger('gpms.grades')
_log_admin = logging.getLogger('gpms.admin')
...
def _refresh_project_statuses():
    ...
    to_update       = []
    newly_completing = []
    for project in Project.objects.filter(team__is_active=True):
...
def student_dashboard(request):
    ...
    _submitted_ms = sum(1 for item in global_milestones if item['submission'])
    _total_ms     = len(global_milestones)
    ...
    _student_analytics = _json.dumps({
        ...
        'completion': {
            'submitted':     _submitted_ms,
            'not_submitted': _total_ms - _submitted_ms,
            'total':         _total_ms,
        },
...
def admin_dashboard(request):
    ...
    active_projects = list(
        Project.objects.filter(team__is_active=True).exclude(status='completed').select_related('team').order_by('title')
    )
    submissions_data = []
    for m in milestones:
        subs = list(
    ...
    grade_projects   = list(
        Project.objects.filter(team__is_active=True).exclude(status='completed')
        .select_related('team', 'supervisor')
        .order_by('team__name')
    )
    ...
    _ms_avg_labels, _ms_avg_values = [], []
    for _gm in grade_milestones:
        _avg = (ProjectGrade.objects
                .filter(milestone=_gm, score__isnull=False, project__in=grade_projects)
                .aggregate(_a=_Avg('score'))['_a'])
        _ms_avg_labels.append(_gm.title)
        _ms_avg_values.append(round(float(_avg), 1) if _avg is not None else None)
    _admin_analytics = _json.dumps({
        ...
        'milestone_averages': {
            'labels': _ms_avg_labels,
            'values': _ms_avg_values,
        },
        'team_status': {
            'active':    _active_t,
            'archived':  _complete_t,
        },
        'totals': {
...
def admin_assign_supervisor(request, project_id):
    ...
    _log_admin.info(
        'SUPERVISOR_ASSIGNED admin=%s supervisor=%s project=%s team=%s',
        request.user.username, supervisor.username, project.title, project.team.name,
    )
...
def admin_remove_supervisor(request, project_id):
    ...
    if old_supervisor:
        _log_admin.info(
            'SUPERVISOR_REMOVED admin=%s supervisor=%s project=%s team=%s',
            request.user.username, old_supervisor.username, project.title, project.team.name,
        )
        Notification.objects.create(
            recipient=old_supervisor,
            type='general',
...
def admin_assign_reviewer(request, project_id):
    ...
    _log_admin.info(
        'REVIEWER_ASSIGNED admin=%s reviewer=%s project=%s team=%s',
        request.user.username, reviewer.username, project.title, project.team.name,
    )
    Notification.objects.create(
        recipient=reviewer,
        type='approval',
...
def admin_remove_reviewer(request, project_id):
    ...
    try:
        ...
        _log_admin.info(
            'REVIEWER_REMOVED admin=%s reviewer=%s project=%s team=%s',
            request.user.username, reviewer.username, project.title, project.team.name,
        )
        Notification.objects.create(
            recipient=reviewer,
...
def admin_create_team(request):
    ...
    _log_admin.info('TEAM_CREATED admin=%s team=%r members=%d', request.user.username, name, len(added))
    if skipped:
...
def admin_create_milestone(request):
    ...
    _log_admin.info('MILESTONE_CREATED admin=%s title=%r due=%s weight=%d', request.user.username, title, due_date, m.weight)
...
def admin_update_milestone(request, milestone_id):
    ...
    _log_admin.info('MILESTONE_UPDATED admin=%s title=%r due=%s weight=%d', request.user.username, m.title, due_date, m.weight)
...
def admin_delete_milestone(request, milestone_id):
    ...
    title = m.title
    m.delete()
    _log_admin.info('MILESTONE_DELETED admin=%s title=%r', request.user.username, title)
...
def admin_switch_phase2(request):
    ...
    _log_admin.info(
        'PHASE2_SWITCH admin=%s auto_teams=%d supervisors_assigned=%d reviewers_assigned=%d no_sup=%d',
        request.user.username, r['auto_teams'], r['assigned_count'], r['reviewer_assigned_count'], r['no_sup_projects'],
    )
...
def admin_revert_phase1(request):
    ...
    _log_admin.info(
        'PHASE1_REVERT admin=%s auto_teams_dissolved=%d supervisors_unassigned=%d',
        request.user.username, dissolved_count, unassigned,
    )
    messages.success(
        request,
...
def student_submit_milestone(request, milestone_id):
    ...
    _log_sub.info(
        'SUBMIT user=%s team=%s project=%s milestone=%s version=%d files=%d late=%s',
        request.user.username, project.team.name, project.title,
        milestone.title, sub.version, len(files), is_late,
    )
