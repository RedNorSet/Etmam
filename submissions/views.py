...
def _parse_grade(raw):
    """Return float 0-100 or None; raises ValueError on bad input."""
    if not raw:
        return None
    g = float(raw)
    if not (0 <= g <= 100):
...
def _recalculate_submission_grade(sub):
    """
    Re-compute sub.grade from all grader component scores, then sync to ProjectGrade.
    - Split milestone: avg each component across graders, then weighted sum.
    - Single milestone: avg all single-component grades across graders.
    """
    ...
    if milestone.has_split:
        ...
        # Sync to ProjectGrade per component
        if sub.project:
        ...
        # Single component: average all graders
        vals = []
...
def supervisor_grade_submission(request, submission_id):
    ...
    if not project or project.supervisor != request.user:
        ...
        return redirect(reverse('dashboard:supervisor') + '#sec-submissions')
    ...
    try:
        if milestone.has_split:
            ...
            # Combined legacy field (weighted)
